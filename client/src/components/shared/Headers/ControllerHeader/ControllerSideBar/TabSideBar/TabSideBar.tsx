...
function TabSideBar({ isFixed }: SideBarProps) {
    ...
    useLayoutEffect(() => {
        const currentTab = (function getCurrentsTabs(tabData: { [x: string]: DataType; }, lvl: number): { [x: string]: DataType; } {
    ...
    const handleReturn = () => {
        dispatch(setActiveTabs(activeTabs.filter((_, i, arr) => i < arr.length - 1)))
    }
    ...
    return <>
        {isFixed && !!activeTabs.length && <div className={Styles.return_button}>
        ...
        <ul className={Styles.tabs}>
            {Object.values(activeTab).map((tab) => {
                ...
                return <DropdownItem title={tab.title} key={uniqId()} onClick={clickAction as any} link={link} highlight />
            })}
        </ul>
    </>;
}
