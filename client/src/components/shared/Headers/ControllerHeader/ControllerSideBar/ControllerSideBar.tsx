...
function ControllerSideBar({ isFixed }: SideBarProp) {
  ...
  const {
    showSideBar: showBar,
    sideBarWrapperStyle: wrapperStyle,
    sideBarStyle: style,
  } = useAppSelector((state) => state.controller.state);
  ...
  const handleToggle = (e?: MouseEvent, hide?: boolean) => {
    if ((showBar && e) || hide) {
      ...
      if (!show) {
        ...
        dispatch(
          setSideBarWrapperStyle({ backgroundColor: "transparent", left: 0 })
        );
  ...
  return (
    <div className={Styles.bar_wrapper}>
      <SideBarIcon className={Styles.icon} onClick={() => handleToggle()} />
      <div
        style={wrapperStyle}
        ref={wrapperRef}
